...
2. **Single-pass expansion.** `expandAbbreviations` selects non-overlapping matches from the original text, leftmost first and longest at the same start, then emits `<span class="dict-expanded">…</span>` wrappers. Generated expansions are never re-matched. **Crucially: a longer key that includes the *expanded* form of a shorter key will not match**, because the shorter key hasn't expanded yet when matching is decided. The longer key must contain the **abbreviated** form.
...
- **Leftmost, then longest.** The earliest valid match wins an overlap; keys at the same start prefer the longest. Thus `"S.E."` wins over the later `"E. of"` in `"S.E. of"`, and contextual `"Origen (Hom"` still beats generic scholar keys.
- **Splits on real HTML tags.** The text is split with `/(<\/?[a-zA-Z][^>]*>)/` and only text segments are matched. Bare scholarly `<` and `>` markers remain text. A key cannot match across a tag boundary. (This is why `convertSupTagsToParens` running first matters.)
- **Unicode boundaries:** word-edge characters use negative lookarounds against `[\p{L}\p{N}\p{M}_]`, including for multiword keys. Punctuation edges need no word boundary, so a trailing period only requires the key's left boundary.
- **`&c.`** is special-cased.
